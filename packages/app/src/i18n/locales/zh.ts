/**
 * 中文（简体）翻译
 * 专业术语不翻译：Skills、Plugins、Commands、Sessions、OpenCode、OpenPackage、OpenWork、MCPs
 */

export default {
  // ==================== Dashboard ====================
  "dashboard.title": "主页",
  "title.dashboard": "主页",
  "title.sessions": "会话",
  "title.scheduled_tasks": "定时任务",
  "title.commands": "命令",
  "title.skills": "技能",
  "title.plugins": "插件",
  "title.mcp": "MCPs",
  "title.settings": "设置",
  "nav.dashboard": "主页",
  "nav.sessions": "会话",
  "nav.scheduled_tasks": "定时任务",
  "nav.commands": "命令",
  "nav.skills": "技能",
  "nav.plugins": "插件",
  "nav.mcp": "MCPs",
  "nav.mcp_alpha": "Alpha",
  "nav.server_offline_hint": "OpenWork 服务器离线 — 远程任务仍可运行。",
  "nav.connect_folder": "连接文件夹",
  "nav.automations": "自动化",
  "nav.soul": "Soul",
  "nav.extensions": "扩展",
  "nav.messaging": "消息",
  "dashboard.sessions": "会话",
  "dashboard.commands": "命令",
  "dashboard.skills": "技能",
  "dashboard.plugins": "插件",
  "dashboard.worker": "工作区",
  "dashboard.sandbox": "沙箱",
  "dashboard.soul_on": "Soul 已启用",
  "dashboard.advanced": "高级",
  "dashboard.mcps": "MCPs",
  "dashboard.settings": "设置",
  "dashboard.home": "主页",
  "dashboard.runs": "运行",
  "dashboard.find_workspace": "查找工作区...",
  "dashboard.workspaces": "工作区",
  "dashboard.no_workspaces": "没有匹配的工作区。",
  "dashboard.new_workspace": "新建工作区...",
  "dashboard.new_remote_workspace": "添加远程工作区...",
  "dashboard.forget_workspace": "忘记工作区",
  "dashboard.remote": "远程",
  "dashboard.connection": "连接",
  "dashboard.local_engine": "本地引擎",
  "dashboard.client_mode": "客户端模式",
  "dashboard.connected": "已连接",
  "dashboard.not_connected": "未连接",
  "dashboard.stop_disconnect": "停止并断开",
  "dashboard.disconnect": "断开连接",
  "dashboard.new_task": "新建任务",
  "dashboard.new": "新建",
  "dashboard.busy": "忙碌",
  "dashboard.hero_title": "今天想做些什么？",
  "dashboard.hero_description": "描述一个目标。OpenWork 将运行它并保留审计记录。",
  "dashboard.task_placeholder": "拟一个要运行的任务...",
  "dashboard.task_aria_label": "描述一个任务",
  "dashboard.run": "运行",
  "dashboard.enter_to_start": "按 Enter 开始新会话。",
  "dashboard.quick_start_commands": "快速开始命令",
  "dashboard.view_all": "查看全部",
  "dashboard.no_commands": "还没有命令。初始命令将显示在这里。",
  "dashboard.run_command": "运行保存的命令",
  "dashboard.recent_sessions": "最近的会话",
  "dashboard.this_workspace": "此工作区",
  "dashboard.no_sessions": "还没有会话。",
  "dashboard.start_task": "开始任务",
  "dashboard.idle": "空闲",
  "dashboard.running": "运行中",
  "dashboard.completed": "已完成",
  "dashboard.failed": "失败",
  "dashboard.export_config": "导出工作区配置",
  "dashboard.export_local_only": "仅本地工作区支持导出",
  "dashboard.share_config": "分享配置",
  "dashboard.add_workspace": "添加工作区",
  "dashboard.copy_path": "复制路径",
  "dashboard.copied": "已复制",
  "dashboard.local": "本地",
  "dashboard.active": "当前",
  "dashboard.switch": "切换",
  "dashboard.switching": "切换中...",
  "dashboard.repairing_cache": "正在修复缓存",
  "dashboard.repair_cache": "修复缓存",
  "dashboard.retry": "重试",
  "dashboard.alpha": "测试版",
  "dashboard.create_workspace_title": "创建工作区",
  "dashboard.create_workspace_subtitle": "初始化新的基于文件夹的工作区。",
  "dashboard.create_workspace_confirm": "创建工作区",
  "dashboard.create_remote_workspace_title": "添加远程工作区",
  "dashboard.create_remote_workspace_subtitle": "保存 OpenWork 服务器为工作区。",
  "dashboard.create_remote_workspace_confirm": "添加工作区",
  "dashboard.create_sandbox_confirm": "创建为沙盒",
  "dashboard.sandbox_get_ready_title": "沙盒需要 Docker",
  "dashboard.sandbox_get_ready_action": "准备沙盒环境",
  "dashboard.sandbox_get_ready_desc": "在隔离的 Docker 容器中运行此工作区，更安全、更可复现。",
  "dashboard.sandbox_checking_docker": "正在检查 Docker...",
  "dashboard.edit_remote_workspace_title": "编辑远程连接",
  "dashboard.edit_remote_workspace_subtitle": "更新此工作区的 OpenWork 服务器信息。",
  "dashboard.edit_remote_workspace_confirm": "保存连接",
  "dashboard.remote_workspace_title": "远程工作区",
  "dashboard.remote_workspace_hint": "记录 OpenWork 服务器，随时重新连接。",
  "dashboard.remote_base_url_label": "OpenWork 服务器地址",
  "dashboard.remote_base_url_placeholder": "http://127.0.0.1:8787",
  "dashboard.remote_base_url_required": "请先填写服务器地址。",
  "dashboard.openwork_host_label": "OpenWork 服务器地址",
  "dashboard.openwork_host_placeholder": "https://your-server.openwork.app",
  "dashboard.openwork_host_hint": "使用 OpenWork 服务器提供的地址。",
  "dashboard.openwork_host_token_label": "访问令牌",
  "dashboard.openwork_host_token_placeholder": "粘贴你的令牌",
  "dashboard.openwork_host_token_hint": "可选。粘贴服务器提供的访问令牌以验证。",
  "dashboard.remote_mode_openwork_alpha": "OpenWork 服务器",
  "dashboard.remote_mode_direct": "直连（旧版）",
  "dashboard.remote_connection_openwork": "OpenWork",
  "dashboard.remote_connection_direct": "直连",
  "dashboard.remote_directory_label": "工作区目录（可选）",
  "dashboard.remote_directory_placeholder": "/home/team/project",
  "dashboard.remote_directory_hint": "留空则使用服务器默认目录。",
  "dashboard.remote_display_name_label": "显示名称（可选）",
  "dashboard.remote_display_name_placeholder": "设计团队工作区",
  "dashboard.select_folder": "选择文件夹",
  "dashboard.choose_folder": "选择文件夹",
  "dashboard.choose_folder_next": "接下来您将选择一个目录。",
  "dashboard.change": "更改",
  "dashboard.opening": "正在打开...",
  "dashboard.choose_preset": "选择预设",
  "dashboard.choose_folder_continue": "选择文件夹以继续。",
  "dashboard.starter_workspace": "启动工作区",
  "dashboard.starter_workspace_desc": "预配置以展示如何使用插件、命令和技能。",
  "dashboard.empty_workspace": "空白工作区",
  "dashboard.empty_workspace_desc": "从空白文件夹开始，添加您需要的内容。",

  // ==================== Soul ====================
  "soul.title": "Soul 与心跳",
  "soul.off_badge": "Soul 已关闭",
  "soul.on_badge": "Soul 已开启",
  "soul.status_not_loaded": "Soul 状态尚未加载。",
  "soul.off_hint": "Soul 当前已关闭。运行一次设置以创建记忆、调度器接线和此工作区的可逆命令。",
  "soul.enable": "启用 Soul 模式",
  "soul.audit_title": "Soul 激活审计",
  "soul.audit_checks": "{n}/6 项检查通过",
  "soul.check.memory": "Soul 记忆文件",
  "soul.check.instructions": "指令接线",
  "soul.check.command": "心跳命令",
  "soul.check.schedule": "心跳调度",
  "soul.check.log": "心跳日志",
  "soul.check.proof": "最近心跳证明",
  "soul.check.waiting": "等待 Soul 状态。",
  "soul.check.proof_hint": "运行一次心跳以验证设置。",
  "soul.heartbeat_stale": "心跳过期",
  "soul.heartbeat_error": "心跳错误",
  "soul.proof_title": "心跳证明",
  "soul.proof_hint": "查看最近的签到、未完成事项和下一步行动。",
  "soul.proof_empty": "暂无心跳记录。立即运行心跳（或 `/soul-heartbeat`）以创建证明。",
  "soul.steering_title": "引导清单",
  "soul.steering_hint": "从此处触发每个引导步骤，并确认 Soul 输出在心跳证明中可见。",
  "soul.check.heartbeat_captured": "心跳已捕获",
  "soul.check.loose_ends": "未完成事项已浮现",
  "soul.check.next_action": "下一步行动已就绪",
  "soul.run_heartbeat": "立即运行心跳",
  "soul.run_heartbeat_running": "正在运行心跳...",
  "soul.heartbeat_completed": "心跳已完成",
  "soul.heartbeat_warning": "心跳仍在运行",
  "soul.heartbeat_in_progress": "心跳进行中",
  "soul.prioritize_loose_ends": "优先处理未完成事项",
  "soul.improvement_sweep": "改进巡检",
  "soul.run_heartbeat_hint": "启动手动心跳并在此卡片查看实时状态。",
  "soul.loose_ends_none": "暂无未完成事项",
  "soul.next_action_hint": "使用引导操作生成一个",
  "soul.worker_label": "工作区",
  "soul.last_heartbeat_label": "最后心跳",
  "soul.heartbeat_count_label": "心跳次数",
  "soul.schedule_label": "调度",
  "soul.no_schedule": "无心跳调度",
  "soul.never": "从未",
  "soul.page_hint": "从此处启用 Soul，审计已接线内容，并在引导下一步行动前验证心跳证明。",
  "soul.loose_ends_tracked": "已追踪 {n} 个未完成事项",
  "soul.loose_ends_tracked_plural": "已追踪 {n} 个未完成事项",

  // ==================== Scheduled Tasks ====================
  "scheduled.title": "定时任务",
  "scheduled.refresh": "刷新",
  "scheduled.refreshing": "正在刷新",
  "scheduled.cards.jobs": "定时任务",
  "scheduled.cards.active_schedules": "活动计划",
  "scheduled.cards.last_sync": "上次同步",
  "scheduled.cards.unavailable": "不可用",
  "scheduled.cards.scheduler": "调度器",
  "scheduled.support.remote_unavailable": "OpenWork 服务器不可用。连接后可同步定时任务。",
  "scheduled.support.desktop_required": "定时任务需要桌面应用。",
  "scheduled.support.windows_unsupported": "Windows 暂不支持调度器。",
  "scheduled.source.remote_description": "来自已连接的 OpenWork 服务器的定时自动化。",
  "scheduled.source.local_description": "在此设备上按计划运行的自动化。",
  "scheduled.source.remote_label": "来自 OpenWork 服务器",
  "scheduled.source.local_label": "来自本地调度器",
  "scheduled.scheduler.remote_label": "OpenWork 服务器",
  "scheduled.scheduler.local_label": "本地",
  "scheduled.scheduler.remote_hint": "远程实例",
  "scheduled.scheduler.local_hint": "Launchd 或 systemd",
  "scheduled.scheduler.remote_unavailable": "OpenWork 服务器不可用",
  "scheduled.scheduler.desktop_only": "仅桌面应用",
  "scheduled.last_synced_never": "尚未同步",
  "scheduled.empty": "还没有定时任务。添加 opencode-scheduler 插件并创建任务后会显示在这里。",
  "scheduled.summary.command": "命令",
  "scheduled.summary.prompt": "提示词",
  "scheduled.summary.task": "任务",
  "scheduled.summary.no_prompt": "未找到提示词或命令。",
  "scheduled.status.not_run": "尚未运行",
  "scheduled.status.running": "运行中",
  "scheduled.status.success": "成功",
  "scheduled.status.failed": "失败",
  "scheduled.cron_label": "Cron",
  "scheduled.run_context": "运行上下文",
  "scheduled.default_workdir": "默认",
  "scheduled.source_label": "来源：",
  "scheduled.last_run": "上次运行",
  "scheduled.created": "创建于",
  "scheduled.agent": "智能体",
  "scheduled.model": "模型",
  "scheduled.delete": "删除",
  "scheduled.delete_title": "删除定时任务？",
  "scheduled.delete_failed": "删除任务失败。",
  "scheduled.deleting": "正在删除",
  "scheduled.cancel": "取消",
  "scheduled.never": "从未",
  "scheduled.delete.remote_description": "这将移除计划，并从已连接的 OpenWork 服务器删除任务定义。",
  "scheduled.delete.local_description": "这将移除计划，并从本机删除任务定义。",

  // ==================== Session ====================
  "session.no_selected": "未选择会话",
  "session.back_to_dashboard": "返回仪表盘",
  "session.back": "返回",
  "session.new_task": "新建任务",
  "session.recents": "最近",
  "session.recents_notice": "这些任务在本地运行，不会跨设备同步。",
  "session.ready_to_work_title": "准备就绪",
  "session.ready_to_work_description": "描述一个任务。我会显示进度并在需要时请求权限。",
  "session.empty_title": "你想做什么？",
  "session.empty_subtitle": "选择一个起点，或直接在下方输入。",
  "session.automate_browser": "自动化浏览器",
  "session.document_label": "文档",
  "session.standard_label": "标准",
  "session.no_artifacts_fallback": "还没有工件。",
  "session.active_plugins_label": "活动插件",
  "session.active_plugins_count": "{count}",
  "session.no_plugins_loaded": "未加载插件。",
  "session.selected_folders_label": "选中的文件夹",
  "session.selected_folders_count": "{count}",
  "session.working_files_label": "工作文件",
  "session.none_yet_label": "还没有。",
  "session.permission_required_title": "需要权限",
  "session.permission_required_description": "OpenCode 正在请求权限以继续。",
  "session.permission_label_uppercase": "权限",
  "session.scope_label_uppercase": "范围",
  "session.details_label": "详情",
  "session.steps_notice_text": "步骤将随着任务的展开而显示。",
  "session.ready_to_work": "准备就绪",
  "session.ready_description": "描述一个任务。我会显示进度并在需要时请求权限。",
  "session.hide_steps": "隐藏步骤",
  "session.view_steps": "查看步骤",
  "session.open": "打开",
  "session.reveal": "在文件管理器中显示",
  "session.opened_toast": "已在默认应用中打开。",
  "session.revealed_toast": "已在文件管理器中显示。",
  "session.artifact_path_missing": "工件路径缺失。",
  "session.desktop_only": "打开功能仅在桌面应用中可用。",
  "session.open_failed": "无法打开工件。",
  "session.model": "模型",
  "session.ready": "就绪",
  "session.connect_provider": "连接提供商以进行自定义。",
  "session.running": "运行中",
  "session.progress": "进度",
  "session.steps_notice": "步骤将随着任务的展开而显示。",
  "session.artifacts": "工件",
  "session.no_artifacts": "还没有工件。",
  "session.context": "上下文",
  "session.active_plugins": "活动插件",
  "session.no_plugins": "未加载插件。",
  "session.selected_folders": "选中的文件夹",
  "session.working_files": "工作文件",
  "session.none_yet": "还没有。",
  "session.document": "文档",
  "session.standard": "标准",
  "session.try_notion_prompt": "立即尝试：在 Notion 中设置我的 CRM",
  "session.insert_prompt": "插入提示",
  "session.placeholder": "向 OpenWork 提问...",
  "session.run": "运行",
  "session.permission_required": "需要权限",
  "session.permission_description": "OpenCode 正在请求权限以继续。",
  "session.permission_label": "权限",
  "session.scope_label": "范围",
  "session.details": "详情",
  "session.deny": "拒绝",
  "session.once": "一次",
  "session.allow_for_session": "在会话期间允许",

  "session.tasks_local_hint": "这些任务在本地运行，不会跨设备同步。",
  "session.recents_label": "最近",
  "session.model_standard": "标准",
  "session.run_button_title": "运行",
  "session.rename_title": "重命名会话",
  "session.rename_description": "更新此会话名称。",
  "session.rename_label": "会话名称",
  "session.rename_placeholder": "输入新的名称",

  // ==================== Commands ====================
  "commands.new": "新建",
  "commands.empty_state": "保存提示词或命令，一键重复运行。",
  "commands.workspace": "此工作区",
  "commands.global": "所有工作区",
  "commands.other": "其他命令",
  "commands.run": "运行",
  "commands.modal_title": "保存可复用流程",
  "commands.modal_description": "保存提示词或命令，便于快速复用。",
  "commands.name_label": "命令名称",
  "commands.name_placeholder": "例如：daily-standup",
  "commands.name_hint": "将在 OpenCode 中显示为 /daily-standup。",
  "commands.description_label": "描述（可选）",
  "commands.description_placeholder": "这个命令做什么？",
  "commands.template_label": "指令",
  "commands.template_placeholder": "编写你想复用的指令…",
  "commands.template_hint": "使用 $ARGUMENTS 来接收详情参数。",
  "commands.details_required": "详情",
  "commands.default_description": "运行保存的命令",
  "commands.command_label": "命令",
  "commands.details_label": "详情",
  "commands.details_placeholder": "添加可选详情",
  "commands.details_hint": "这些详情将传给命令。",
  "commands.run_modal_title": "运行命令",
  "commands.run_modal_description": "运行前可添加详情。",
  "commands.run_modal_run": "运行命令",
  "commands.name_will_be": "将创建为",
  "commands.override_title": "替换现有命令？",
  "commands.override_description": "已存在同名命令。",
  "commands.override_warning": "这将替换现有的 \"{name}\" 命令。此操作无法撤销。",
  "commands.override_confirm": "替换",
  "commands.override_cancel": "保留现有",

  // ==================== Skills ====================
  "skills.title": "Skills",
  "skills.subtitle": "管理此工作区的 skills。",
  "skills.refresh": "刷新",
  "skills.add_title": "添加 skills",
  "skills.add_description": "安装起始命令、导入 skill，或打开文件夹。",
  "skills.install_from_openpackage": "从 OpenPackage 安装",
  "skills.host_mode_only": "仅本地工作区",
  "skills.install": "安装",
  "skills.installed_label": "已安装",
  "skills.install_hint": "将 OpenPackage 包安装到当前工作区。Skills 应放在 `.opencode/skills` 中。",
  "skills.import_local": "导入本地 skill",
  "skills.import_local_hint": "将现有 skill 文件夹复制到此工作区。",
  "skills.import": "导入",
  "skills.curated_packages": "精选包",
  "skills.view": "查看",
  "skills.search_placeholder": "搜索已安装或 Hub 技能",
  "skills.no_matches": "没有匹配的精选包。尝试不同的搜索。",
  "skills.install_package": "安装",
  "skills.registry_notice": "发布到 OpenPackage 注册表（`opkg push`）目前需要身份验证。计划添加注册表搜索和精选列表同步。",
  "skills.installed": "已安装的 skills",
  "skills.no_skills": "在 `.opencode/skills`、`.claude/skills` 或 `~/.agents/skills` 中未检测到 skills。",
  "skills.desktop_required": "技能管理需要桌面应用。",
  "skills.host_only_error": "技能管理需要本地工作区或已连接的 OpenWork 服务器。",
  "skills.install_skill_creator": "安装 skill creator",
  "skills.install_skill_creator_hint": "此 skill 可让你在聊天中创建其他 skills。",
  "skills.installing_skill_creator": "正在安装 skill creator...",
  "skills.skill_creator_installed": "Skill creator 已安装。",
  "skills.skill_creator_already_installed": "Skill creator 已经安装过了。",
  "skills.install_failed": "技能安装失败。",
  "skills.reveal_folder": "打开 skills 文件夹",
  "skills.reveal_folder_hint": "在 Finder 中打开 skills 目录。",
  "skills.reveal_button": "在 Finder 中显示",
  "skills.reveal_failed": "打开 skills 文件夹失败。",
  "skills.uninstall": "卸载",
  "skills.uninstall_title": "卸载 skill？",
  "skills.uninstall_warning": "这将永久从工作区删除 `{name}` skill。",
  "skills.uninstall_failed": "卸载 skill 失败。",
  "skills.uninstalled": "Skill 已删除。",
  "skills.source_placeholder": "github:anthropics/claude-code",
  "skills.notion_crm_title": "Notion CRM 增强技能",
  "skills.notion_crm_description": "为联系人、管道和跟进添加增强工作流。",
  "skills.notion_crm_card_description": "使用现成的技能丰富 Notion CRM 数据。",
  "skills.connect_host_to_load": "连接 OpenWork 服务器以加载技能。",
  "skills.pick_workspace_first": "先选择一个工作区文件夹。",
  "skills.no_skills_found": "还没有找到 skills。",
  "skills.installed_description": "此工作区可用的 skills。",
  "skills.failed_to_load": "加载 skills 失败",
  "skills.plugin_management_host_only": "插件管理需要桌面应用。",
  "skills.plugins_host_only": "插件仅在桌面应用中可用。",
  "skills.pick_project_for_plugins": "选择项目文件夹以管理项目插件。",
  "skills.pick_project_for_active": "选择项目文件夹以加载活动插件。",
  "skills.no_opencode_found": "尚未找到 opencode.json。添加插件以创建一个。",
  "skills.no_opencode_workspace": "此工作区中还没有 opencode.json。",
  "skills.failed_parse_opencode": "解析 opencode.json 失败",
  "skills.failed_load_opencode": "加载 opencode.json 失败",
  "skills.failed_load_active": "加载活动插件失败。",
  "skills.enter_plugin_name": "输入插件包名称。",
  "skills.plugin_already_listed": "插件已在 opencode.json 中列出。",
  "skills.failed_update_opencode": "更新 opencode.json 失败",
  "skills.opackage_install_host_only": "OpenPackage 安装需要桌面应用。",
  "skills.pick_project_first": "先选择一个项目文件夹。",
  "skills.enter_opackage_source": "输入 OpenPackage 源（例如：github:anthropics/claude-code）。",
  "skills.installing_opackage": "正在安装 OpenPackage...",
  "skills.install_complete": "已安装。",
  "skills.curated_list_notice": "这是一个精选列表，还不是 OpenPackage。复制链接或关注 PRD 以了解计划的注册表搜索集成。",
  "skills.import_host_only": "Skill 导入需要桌面应用。",
  "skills.select_skill_folder": "选择 skill 文件夹",
  "skills.import_failed": "导入失败（{status}）",
  "skills.imported": "已导入。",
  "skills.unknown_error": "未知错误",
  "skills.worker_profile": "工作区配置",
  "skills.worker_description": "技能是此工作区的核心能力。从 Hub 添加或直接在对话中创建新技能。",
  "skills.create_in_chat": "在对话中创建技能",
  "skills.hub_available": "Hub 可用",
  "skills.creator_label": "技能创建器",
  "skills.not_installed": "未安装",
  "skills.mode_label": "模式",
  "skills.mode_local": "本地",
  "skills.mode_server": "服务器",
  "skills.new_skill": "新建技能",
  "skills.install_from_link": "从链接安装",
  "skills.install_from_link_title": "从链接安装技能",
  "skills.share_link": "分享链接",
  "skills.refresh_hub": "刷新 Hub 目录",
  "skills.installing": "安装中",
  "skills.add": "添加",
  "skills.link_copied": "链接已复制",
  "skills.paste_link": "粘贴链接以预览",
  "skills.publishing": "发布中…",
  "skills.create_link": "创建链接",
  "skills.loading": "加载中…",
  "skills.preview": "预览",
  "skills.installing_btn": "安装中…",
  "skills.keep_both": "保留两者",
  "skills.overwrite": "覆盖",
  "skills.install_name": "安装 {name}",
  "skills.trigger_label": "触发词",
  "skills.hub_section_title": "Hub 技能",

  // ==================== Plugins ====================
  "plugins.title": "OpenCode 插件",
  "plugins.description": "为您的项目或全局 OpenCode 插件管理 `opencode.json`。",
  "plugins.config_label": "配置",
  "plugins.config_not_loaded": "尚未加载",
  "plugins.suggested_label": "建议的插件",
  "plugins.no_plugins_yet": "尚未配置插件。",
  "plugins.enabled_label": "已启用",
  "plugins.open_label": "打开：",
  "plugins.path_label": "路径：",
  "plugins.scope_project": "项目",
  "plugins.scope_global": "全局",
  "plugins.refresh": "刷新",
  "plugins.config": "配置",
  "plugins.not_loaded": "尚未加载",
  "plugins.suggested": "建议的插件",
  "plugins.hide_setup": "隐藏设置",
  "plugins.setup": "设置",
  "plugins.added": "已添加",
  "plugins.add": "添加",
  "plugins.enabled": "已启用",
  "plugins.no_plugins": "尚未配置插件。",
  "plugins.add_label": "添加插件",
  "plugins.placeholder": "opencode-wakatime",
  "plugins.add_hint": "添加 npm 包名称，例如 opencode-wakatime",

  // ==================== MCP (Model Context Protocol) ====================
  "mcp.title": "MCP（测试版）",
  "mcp.description": "MCP 服务器让您使用自己的凭据连接服务。",
  "mcp.alpha_banner_title": "MCP 处于测试阶段，我们正在加强与 OpenCode 的 OAuth 集成。",
  "mcp.alpha_banner_help": "如果您想帮忙，请提交 PR 并附上一个短视频，展示 OAuth 流程从端到端的工作。",
  "mcp.mcps_title": "MCPs",
  "mcp.connect_mcp_hint": "连接 MCP 服务器以扩展 OpenWork 的功能。",
  "mcp.reload_banner_title": "需要重新加载",
  "mcp.reload_banner_description": "更改需要快速重新加载以激活 MCP 工具。",
  "mcp.reload_banner_description_blocked": "此工作区中有任务正在运行。请先停止，然后重新加载以完成设置。",
  "mcp.reload_banner_blocked_hint": "停止活动运行以重新加载。",
  "mcp.quick_connect_title": "快速连接",
  "mcp.oauth_only_label": "OAuth + 本地",
  "mcp.connected_status": "已连接",
  "mcp.no_env_vars": "不需要环境变量。",
  "mcp.connected_title": "已连接",
  "mcp.from_opencode_json": "来自 opencode.json",
  "mcp.no_servers_yet": "尚未配置 MCP 服务器。",
  "mcp.edit_config_title": "编辑 MCP 配置",
  "mcp.edit_config_description": "MCP 服务器位于 OpenCode 的 opencode.json 中。",
  "mcp.docs_link": "文档",
  "mcp.scope_project": "项目",
  "mcp.scope_global": "全局",
  "mcp.config_label": "配置",
  "mcp.config_not_loaded": "尚未加载",
  "mcp.open_file_label": "打开文件",
  "mcp.reveal_in_finder": "在文件管理器中显示",
  "mcp.opening_label": "正在打开",
  "mcp.file_not_found": "文件未找到",
  "mcp.config_load_failed": "加载配置路径失败",
  "mcp.open_file": "打开文件",
  "mcp.pick_workspace_error": "选择工作区文件夹以显示项目 opencode.json。",
  "mcp.reveal_config_failed": "显示配置失败",
  "mcp.alpha_warning": "MCP 处于测试阶段，我们正在加强与 OpenCode 的 OAuth 集成。",
  "mcp.github_issue": "在 GitHub 上查看 issue #9510",
  "mcp.contribution_guide": "如果您想帮忙，请提交 PR 并附上一个短视频，展示 OAuth 流程从端到端的工作。",
  "mcp.hide_advanced": "隐藏高级设置",
  "mcp.show_advanced": "显示高级设置",
  "mcp.mcps_label": "MCPs",
  "mcp.mcps_description": "连接 MCP 服务器以扩展 OpenWork 的功能。",
  "mcp.configured": "已配置",
  "mcp.updated": "更新时间",
  "mcp.reload_required": "需要重新加载",
  "mcp.reload_description": "更改需要快速重新加载以激活 MCP 工具。",
  "mcp.reload_engine": "重新加载引擎",
  "mcp.quick_connect": "快速连接",
  "mcp.oauth_only": "仅 OAuth",
  "mcp.connecting": "连接中",
  "mcp.connect": "连接",
  "mcp.connected": "已连接",
  "mcp.connected_label": "已连接",
  "mcp.no_env_required": "不需要环境变量。",
  "mcp.config_source": "来自 opencode.json",
  "mcp.no_servers": "尚未配置 MCP 服务器。",
  "mcp.advanced": "高级",
  "mcp.advanced_description": "手动设置自定义服务器。",
  "mcp.hide": "隐藏",
  "mcp.show": "显示",
  "mcp.server_name": "服务器名称",
  "mcp.server_name_placeholder": "sentry",
  "mcp.server_url": "服务器 URL",
  "mcp.server_url_placeholder": "https://mcp.sentry.dev/mcp",
  "mcp.oauth": "OAuth",
  "mcp.api_key": "API 密钥",
  "mcp.enabled": "已启用",
  "mcp.disabled": "已禁用",
  "mcp.add_mcp": "添加 MCP",
  "mcp.verify_connection": "验证连接",
  "mcp.cli_guidance": "CLI 指南（从您的工作区运行）",
  "mcp.config_locations": "配置可以位于 opencode.json、opencode.jsonc 或 .opencode/opencode.json。",
  "mcp.details_title": "详情",
  "mcp.select_server_hint": "选择服务器以查看状态和配置。",
  "mcp.capabilities_label": "功能",
  "mcp.tools_enabled_label": "已启用工具",
  "mcp.oauth_ready_label": "OAuth 就绪",
  "mcp.usage_hint_text": "在提示中使用 MCP 服务器名称来定位其工具。",
  "mcp.next_steps_label": "后续步骤",
  "mcp.reload_step": "添加服务器后重新加载引擎。",
  "mcp.auth_step": "如果提示，为 OAuth 服务器运行 opencode mcp auth。",
  "mcp.connection_failed": "连接失败",
  "mcp.needs_auth": "需要认证",
  "mcp.register_client": "注册客户端",
  "mcp.status_disabled": "已禁用",
  "mcp.disconnected": "已断开",
  "mcp.failed": "失败",
  "mcp.host_mode_only": "MCP 连接需要桌面应用。",
  "mcp.pick_workspace_first": "先选择一个工作区文件夹。",
  "mcp.desktop_required": "MCP 连接需要桌面应用。",
  "mcp.connect_server_first": "先连接到 OpenCode 服务器。",
  "mcp.reload_required_after_add": "添加 MCP 后需要重新加载。",
  "mcp.connect_failed": "MCP 连接失败。",
  "mcp.enter_name_and_url": "输入服务器名称和 URL。",
  "mcp.enter_url_first": "先输入服务器 URL。",
  "mcp.use_debug_command": "使用 opencode mcp debug <name> 验证连接。",
  "mcp.add_failed": "添加 MCP 失败。",
  "mcp.apps_title": "应用",
  "mcp.apps_subtitle": "连接你常用的工具，让 OpenWork 代你操作。",
  "mcp.app_connected": "个应用已连接",
  "mcp.apps_connected": "个应用已连接",
  "mcp.finish_setup": "即将完成",
  "mcp.finish_setup_hint": "点击「激活」完成应用连接。",
  "mcp.activate_button": "激活",
  "mcp.available_apps": "可用应用",
  "mcp.one_click_connect": "一键连接",
  "mcp.tap_to_connect": "点击连接",
  "mcp.connected_badge": "已连接",
  "mcp.your_apps": "我的应用",
  "mcp.last_synced": "已同步",
  "mcp.no_apps_yet": "还没有连接任何应用",
  "mcp.no_apps_hint": "在上方连接一个开始使用。",
  "mcp.config_file": "配置文件",
  "mcp.advanced_settings": "高级设置",
  "mcp.advanced_settings_hint": "手动编辑配置文件和管理连接。",
  "mcp.app_details": "应用详情",
  "mcp.select_app_hint": "选择一个应用查看详情。",
  "mcp.connection_type": "连接类型",
  "mcp.type_cloud": "云端（使用账号登录）",
  "mcp.type_local": "本地（在此设备运行）",
  "mcp.cap_tools": "AI 工具",
  "mcp.cap_signin": "账号登录",
  "mcp.issue_label": "问题",
  "mcp.technical_details": "技术详情",
  "mcp.friendly_status_ready": "就绪",
  "mcp.friendly_status_needs_signin": "需要登录",
  "mcp.friendly_status_paused": "已暂停",
  "mcp.friendly_status_offline": "离线",
  "mcp.friendly_status_issue": "有问题",
  "mcp.remove_app": "移除",
  "mcp.remove_failed": "移除应用失败。",
  "mcp.remove_modal_title": "移除应用",
  "mcp.remove_modal_message": "确定要移除 {server}？你随时可以重新添加。",
  "mcp.add_modal_title": "添加 MCP 服务器",
  "mcp.add_modal_subtitle": "通过 URL 或本地命令连接自定义 MCP 服务器。",
  "mcp.server_type": "类型",
  "mcp.type_remote": "远程（URL）",
  "mcp.type_local_cmd": "本地（命令）",
  "mcp.server_command": "命令",
  "mcp.server_command_placeholder": "npx -y @modelcontextprotocol/server-sequential-thinking",
  "mcp.server_command_hint": "启动服务器的 shell 命令。",
  "mcp.add_server_button": "添加服务器",
  "mcp.name_required": "请输入服务器名称。",
  "mcp.url_or_command_required": "远程服务器请输入 URL，本地服务器请输入命令。",
  "mcp.logout_label": "OAuth",
  "mcp.logout_action": "退出登录",
  "mcp.logout_working": "正在退出...",
  "mcp.logout_hint": "移除已保存的 OAuth 凭据，下次使用需重新登录。",
  "mcp.logout_modal_title": "退出此应用？",
  "mcp.logout_modal_message": "这将移除 {server} 的已保存 OAuth 凭据，下次使用需重新登录。",
  "mcp.logout_success": "已退出 {server}。",
  "mcp.logout_failed": "退出登录失败。",

  // MCP Auth Modal
  "mcp.auth.open_browser_signin": "我们将打开您的浏览器完成登录。",
  "mcp.auth.connect_server": "连接 {server}",
  "mcp.auth.already_connected": "已连接",
  "mcp.auth.already_connected_description": "{server} 已通过身份验证，可以正常使用。",
  "mcp.auth.configured_previously": "该 MCP 可能在全局或之前的会话中已配置。您可以关闭此弹窗，立即开始使用 MCP 工具。",
  "mcp.auth.reload_engine_retry": "重新加载引擎并重试",
  "mcp.auth.retry_now": "立即重试",
  "mcp.auth.retry": "重试",
  "mcp.auth.reload_before_oauth": "在开始 OAuth 之前，请重新加载引擎以完成此 MCP 的设置。",
  "mcp.auth.reload_notice": "重新加载引擎以激活此 MCP，完成设置。这是必要步骤，不是错误。",
  "mcp.auth.reload_blocked": "会话运行期间重新加载已暂停。请先停止运行再完成设置。",
  "mcp.auth.reload_needed": "重新加载引擎完成设置后，再次尝试连接。",
  "mcp.auth.manual_finish_title": "远程服务器？",
  "mcp.auth.manual_finish_hint": "粘贴回调 URL（localhost:19876）或仅粘贴代码以完成连接。",
  "mcp.auth.callback_label": "回调 URL 或代码",
  "mcp.auth.callback_placeholder": "http://127.0.0.1:19876/mcp/oauth/callback?code=...",
  "mcp.auth.complete_connection": "完成连接",
  "mcp.auth.callback_invalid": "粘贴回调 URL 或 code 参数以完成 OAuth。",
  "mcp.auth.port_forward_hint": "提示：如需转发回调端口：ssh -L 19876:127.0.0.1:19876 user@host",
  "mcp.auth.step1_title": "正在打开您的浏览器",
  "mcp.auth.step1_description": "我们将自动启动 {server} 的登录流程。",
  "mcp.auth.step2_title": "授权 OpenWork",
  "mcp.auth.step2_description": "登录并在提示时批准访问权限。",
  "mcp.auth.step3_title": "完成后返回此处",
  "mcp.auth.step3_description": "授权完成后我们将立即完成连接。",
  "mcp.auth.waiting_authorization": "正在等待您在浏览器中完成授权...",
  "mcp.auth.done": "完成",
  "mcp.auth.cancel": "取消",
  "mcp.auth.im_done": "我已完成",
  "mcp.auth.client_registration_required": "在继续 OAuth 之前需要先注册客户端。",
  "mcp.auth.server_disabled": "此 MCP 服务器已禁用。请启用它后重试。",
  "mcp.auth.oauth_failed": "OAuth 身份验证失败。",
  "mcp.auth.invalid_refresh_token": "OAuth 刷新令牌无效或已过期。请重新授权。",
  "mcp.auth.reauth_action": "重新授权 OAuth",
  "mcp.auth.reauth_running": "正在重新授权...",
  "mcp.auth.reauth_failed": "重新授权失败。",
  "mcp.auth.reauth_cli_hint": "运行：opencode mcp auth {server}",
  "mcp.auth.reauth_remote_hint": "请在运行该工作区的设备上重新授权。",
  "mcp.auth.authorization_still_required": "仍需要授权。请重试以重新启动流程。",
  "mcp.auth.oauth_not_supported_hint": "这可能意味着：\n• MCP 服务器未声明 OAuth 功能\n• 引擎需要重新加载以发现服务器功能\n• 尝试：从 CLI 运行 opencode mcp auth {server}",
  "mcp.auth.try_reload_engine": "{message}。请尝试先重新加载引擎。",
  "mcp.auth.failed_to_start_oauth": "启动 OAuth 流程失败",
  "mcp.auth.oauth_completed_reload": "OAuth 已完成。重新加载引擎以激活 MCP。",

  // ==================== Settings ====================
  "settings.title": "设置",
  "settings.connection": "连接",
  "settings.from_path": "从 PATH",
  "settings.from_sidecar": "内置 sidecar",
  "settings.engine_source_description": "PATH 使用您安装的 OpenCode（默认）。Sidecar 将在可用时使用内置的二进制文件。",
  "settings.sidecar_unsupported": "Sidecar 已支持 Windows",
  "settings.sidecar_unavailable_detail": "Sidecar 可用时会随应用内置。",
  "settings.model": "模型",
  "settings.model_description": "默认值 + 运行的思考控制。",
  "settings.change": "更改",
  "settings.engine_path": "PATH",
  "settings.engine_sidecar": "Sidecar",
  "settings.thinking": "思考",
  "settings.thinking_description": "显示思考部分（仅开发者模式）。",
  "settings.on": "开启",
  "settings.off": "关闭",
  "settings.model_variant": "模型变体",
  "settings.edit": "编辑",
  "settings.default_model": "默认模型",
  "settings.session_model": "模型",
  "settings.model_description_default": "从您配置的提供商中选择。此选择将用于新会话。",
  "settings.model_description_session": "从您配置的提供商中选择。此选择适用于您的下一条消息。",
  "settings.search_models": "搜索模型…",
  "settings.showing_models": "显示 {count} / {total}",
  "settings.model_variant_prompt": "模型变体（提供商特定，例如 high/max/minimal）。留空则清除。",
  "settings.model_fallback": "备用",
  "settings.model_default": "默认",
  "settings.model_free": "免费",
  "settings.model_reasoning": "推理",
  "settings.done": "完成",
  "settings.updates": "更新",
  "settings.updates_description": "保持 OpenWork 为最新版本。",
  "settings.automatic_checks": "自动检查",
  "settings.automatic_checks_description": "每天一次（安静）",
  "settings.update_checking": "检查中...",
  "settings.update_available": "可用更新 {version}",
  "settings.update_downloading": "下载中...",
  "settings.update_ready": "准备安装 {version}",
  "settings.update_error": "更新检查失败",
  "settings.update_failed": "更新检查失败",
  "settings.update_uptodate": "已是最新",
  "settings.last_checked": "上次检查",
  "settings.published": "发布时间",
  "settings.check_update": "检查",
  "settings.install_restart": "安装并重启",
  "settings.update_not_supported": "此环境不支持更新。",
  "settings.update_desktop_only": "更新仅在桌面应用中可用。",
  "settings.startup": "启动",
  "settings.mode_label": "模式",
  "settings.switch_mode": "切换",
  "settings.reset_startup": "重置默认启动模式",
  "settings.reset_startup_description": "这将清除您保存的偏好设置，并在下次启动时显示模式选择。",
  "settings.advanced": "高级",
  "settings.advanced_description": "重置 OpenWork 本地状态以重新测试入门流程。",
  "settings.reset_onboarding": "重置入门",
  "settings.reset_onboarding_description": "清除 OpenCode 偏好设置并重启应用。",
  "settings.reset_app_data": "重置应用数据",
  "settings.reset_app_data_description": "更激进的方式。清除 OpenCode 缓存 + 应用数据。",
  "settings.reset": "重置",
  "settings.requires_typing": "需要输入",
  "settings.will_restart": "并将重启应用。",
  "settings.reset_onboarding_title": "重置入门",
  "settings.reset_app_data_title": "重置应用数据",
  "settings.reset_confirmation_hint": "输入 <span class=\"font-mono\">RESET</span> 以确认。OpenWork 将重启。",
  "settings.reset_onboarding_warning": "清除 OpenWork 本地偏好设置和工作区入门标记。",
  "settings.reset_app_data_warning": "清除此设备上的 OpenWork 缓存和应用数据。",
  "settings.reset_stop_active_runs": "重置前停止活动运行。",
  "settings.reset_confirmation_label": "确认",
  "settings.reset_confirmation_placeholder": "输入 RESET",
  "settings.reset_cancel": "取消",
  "settings.reset_confirm_button": "重置并重启",
  "settings.developer": "开发者",
  "settings.opencode_cache": "OpenCode 缓存",
  "settings.opencode_cache_description": "修复用于启动引擎的缓存数据。安全运行。",
  "settings.repair_cache": "修复缓存",
  "settings.repairing_cache": "正在修复缓存",
  "settings.cache_repair_requires_desktop": "缓存修复需要桌面应用",
  "settings.migration_recovery_label": "数据迁移恢复",
  "settings.migration_recovery_hint": "如果从旧版 JSON 数据迁移时本地启动失败，请使用此功能。",
  "settings.fix_migration": "修复迁移",
  "settings.fixing_migration": "正在修复迁移...",
  "settings.migration_repair_requires_desktop": "迁移修复需要桌面应用",
  "settings.pending_permissions": "待处理的权限",
  "settings.recent_events": "最近的事件",
  "settings.notion_connected": "已连接",
  "settings.reload_required": "需要重新加载",
  "settings.connection_failed": "连接失败",
  "settings.notion_not_connected": "未连接",
  "settings.show_thinking": "显示思考过程",
  "settings.update": "更新",
  "settings.about": "关于",
  "settings.version": "版本",
  "settings.check_for_updates": "检查更新",
  "settings.download_update": "下载更新",
  "settings.install_update": "安装更新并重启",
  "settings.enable_developer_mode": "启用开发者模式",
  "settings.disable_developer_mode": "禁用开发者模式",
  "settings.stop_local_server": "停止本地服务器",
  "settings.stop_engine": "停止引擎",
  "settings.disconnect": "断开连接",
  "settings.disconnect_server": "断开服务器",
  "settings.language": "语言",
  "settings.language.description": "选择您的首选语言",
  "settings.connection_title": "连接",
  "settings.providers_title": "服务提供商",
  "settings.providers_hint": "连接模型和工具所需的服务。",
  "settings.providers_loading": "正在加载服务提供商...",
  "settings.providers_connect": "连接服务提供商",
  "settings.providers_footer": "API 密钥由 OpenCode 本地保存。使用 /models 选择默认模型。",
  "settings.providers_status_unavailable": "不可用",
  "settings.providers_status_disconnected": "未连接",
  "settings.providers_status_connected": "已连接 {count}",
  "settings.providers_status_hint": "连接 OpenCode 以加载服务提供商。",
  "settings.providers_available_only": "可用 {count}",
  "settings.providers_connected_summary": "已连接 {connected} · 可用 {available}",
  "settings.providers_open_failed": "打开服务提供商失败",
  "settings.remote_sharing_title": "OpenWork 服务器共享",
  "settings.remote_sharing_hint": "将这些信息分享给受信任的设备。保持同一网络以获得最快连接。",
  "settings.remote_server_url_label": "OpenWork 服务器地址",
  "settings.remote_server_url_starting": "正在启动服务器…",
  "settings.remote_mdns_hint": ".local 名称更易记，但在某些网络可能无法解析。",
  "settings.remote_local_ip_hint": "同一 Wi‑Fi 下使用本地 IP 可获得最快连接。",
  "settings.remote_copy": "复制",
  "settings.remote_copied": "已复制",
  "settings.remote_access_token_label": "访问令牌",
  "settings.remote_access_token_hint": "供连接此服务器的手机或笔记本使用。",
  "settings.remote_server_token_label": "服务器令牌",
  "settings.remote_server_token_hint": "请妥善保管。审批操作需要该令牌。",
  "settings.remote_openwork_title": "OpenWork 服务器",
  "settings.remote_openwork_hint": "连接到 OpenWork 服务器。使用管理员提供的地址和访问令牌。",
  "settings.remote_openwork_url_label": "OpenWork 服务器地址",
  "settings.remote_openwork_url_placeholder": "http://127.0.0.1:8787",
  "settings.remote_openwork_url_hint": "使用 OpenWork 服务器分享的地址。",
  "settings.remote_openwork_token_placeholder": "粘贴你的令牌",
  "settings.remote_openwork_token_hint": "可选。粘贴访问令牌以验证。",
  "settings.remote_resolved_server": "解析后的服务器：",
  "settings.remote_not_set": "未设置",
  "settings.remote_test_connection": "测试连接",
  "settings.remote_testing": "测试中...",
  "settings.remote_test_success": "连接成功。",
  "settings.remote_test_failed": "连接失败。请检查服务器地址和令牌。",
  "settings.remote_test_failed_generic": "连接失败。",
  "settings.remote_testing_status": "正在测试连接...",
  "settings.remote_status_updated": "连接状态已更新。",
  "settings.remote_sync_hint": "需要连接 OpenWork 服务器以同步技能、插件和命令。",
  "settings.remote_engine_reload_title": "引擎重载",
  "settings.remote_engine_reload_hint": "重启此工作区的 OpenCode 服务器。",
  "settings.remote_reload_now": "立即重载",
  "settings.remote_reload_hint": "应用配置更新并重新连接会话。",
  "settings.remote_reload_warning": "重载会停止正在运行的任务。",
  "settings.remote_reload_requires_connection": "连接到此工作区后才能重载。",
  "settings.remote_reload_requires_local": "仅本地工作区或已连接的 OpenWork 服务器可重载。",
  "settings.remote_reloading": "正在重载...",
  "settings.remote_reload_engine": "重载引擎",
  "settings.remote_status_connected": "已连接",
  "settings.remote_status_limited": "受限",
  "settings.remote_status_disconnected": "未连接",
  "settings.remote_status_offline": "离线",
  "settings.remote_status_available": "可用",
  "settings.startup_title": "启动",
  "settings.reset_startup_preference": "重置启动偏好",
  "settings.reset_startup_hint": "清除保存的偏好，下次启动会显示连接选择。",
  "settings.engine_title": "引擎",
  "settings.engine_hint": "选择 OpenCode 在本地的运行方式。",
  "settings.engine_source": "引擎来源",
  "settings.engine_source_bundled": "内置（推荐）",
  "settings.engine_source_system": "系统安装（PATH）",
  "settings.engine_source_hint": "内置引擎最稳定，仅当你自行管理 OpenCode 时使用系统安装。",
  "settings.engine_runtime": "引擎运行时",
  "settings.engine_runtime_direct": "直连（OpenCode）",
  "settings.engine_runtime_openwrk": "Openwrk 调度器",
  "settings.engine_runtime_hint": "下次引擎启动或重载时生效。",
  "settings.reset_title": "重置与恢复",
  "settings.reset_hint": "清理数据或重新走初始化流程。",
  "settings.reset_warning": "需要输入 RESET 并将重启应用。",
  "settings.engine_source_label": "引擎来源",
  "settings.sidecar_unavailable": "Sidecar 可用时会随应用内置。",
  "settings.model_title": "模型",
  "settings.model_hint": "默认值 + 运行的思考控制。",
  "settings.thinking_label": "思考",
  "settings.thinking_hint": "显示思考部分（仅开发者模式）。",
  "settings.model_variant_label": "模型变体",
  "settings.appearance_title": "外观",
  "settings.appearance_hint": "匹配系统或强制浅色/深色模式。",
  "settings.theme_system": "系统",
  "settings.theme_light": "浅色",
  "settings.theme_dark": "深色",
  "settings.theme_system_hint": "系统模式自动跟随您的操作系统偏好。",
  "settings.updates_title": "更新",
  "settings.updates_hint": "保持 OpenWork 为最新版本。",
  "settings.automatic_checks_label": "自动检查",
  "settings.automatic_checks_hint": "每天一次（安静）",
  "settings.last_checked_time": "上次检查 {time}",
  "settings.published_date": "发布时间 {date}",
  "settings.update_not_supported_hint": "此环境不支持更新。",
  "settings.update_desktop_only_hint": "更新仅在桌面应用中可用。",
  "settings.mode_suffix": "模式",
  "settings.reset_startup_label": "重置默认启动模式",
  "settings.advanced_title": "高级",
  "settings.advanced_hint": "重置 OpenWork 本地状态以重新测试入门流程。",
  "settings.reset_onboarding_label": "重置入门",
  "settings.reset_onboarding_hint": "清除 OpenCode 偏好设置并重启应用。",
  "settings.reset_app_data_label": "重置应用数据",
  "settings.reset_app_data_hint": "更激进的方式。清除 OpenCode 缓存 + 应用数据。",
  "settings.reset_requires_hint": "需要输入 RESET 并将重启应用。",
  "settings.developer_title": "开发者",
  "settings.opencode_cache_label": "OpenCode 缓存",
  "settings.opencode_cache_hint": "修复用于启动引擎的缓存数据。安全运行。",
  "settings.pending_permissions_label": "待处理的权限",
  "settings.recent_events_label": "最近的事件",
  "settings.stop_active_runs_hint": "停止活动运行以更新",
  "settings.stop_active_runs_reset_hint": "停止活动运行以重置",
  "settings.stop_runs_to_update": "停止活动运行以更新",
  "settings.stop_runs_to_reset": "停止活动运行以重置",
  "settings.updates_not_supported": "此环境不支持更新。",
  "settings.updates_desktop_only": "更新仅在桌面应用中可用。",
  "settings.status.unavailable": "不可用",
  "settings.status.running": "运行中",
  "settings.status.offline": "离线",
  "settings.status.loading": "加载中",
  "settings.status.error": "错误",
  "settings.status.ready": "就绪",
  "settings.startup_local": "启动本地服务器",
  "settings.startup_server": "连接服务器",
  "settings.startup_unset": "未设置",
  "settings.tab_general": "常规",
  "settings.tab_model": "模型",
  "settings.tab_keybinds": "快捷键",
  "settings.tab_advanced": "高级",
  "settings.tab_remote": "远程",
  "settings.tab_messaging": "消息桥接",
  "settings.tab_debug": "调试",
  "settings.actor.unknown": "未知",
  "settings.actor.host": "主机",
  "settings.actor.remote": "远程",
  "settings.actor.remote_with_id": "远程:{id}",
  "settings.capability.unavailable": "不可用",
  "settings.capability.read": "读",
  "settings.capability.write": "写",
  "settings.capability.none": "无权限",
  "settings.logs.desktop_only": "仅桌面应用可用。",
  "settings.logs.host_only": "日志在主机上可用。",
  "settings.logs.no_stdout": "暂无 stdout。",
  "settings.logs.no_stderr": "暂无 stderr。",
  "settings.messaging.title": "消息桥接",
  "settings.messaging.subtitle": "连接 Telegram 和 WhatsApp 与 AI 对话。",
  "settings.messaging.saving": "正在保存...",
  "settings.messaging.enabled": "已启用",
  "settings.messaging.disabled": "已禁用",
  "settings.messaging.alpha": "测试版",
  "settings.messaging.note": "消息桥接将 WhatsApp 与 Telegram 连接到 OpenCode，消息在本地处理。",
  "settings.messaging.telegram.configured": "已配置",
  "settings.messaging.telegram.not_configured": "未配置",
  "settings.messaging.telegram.bot_token": "机器人令牌",
  "settings.messaging.telegram.placeholder": "粘贴来自 @BotFather 的令牌",
  "settings.messaging.telegram.help": "在 Telegram 中通过 @BotFather 创建机器人并将令牌粘贴到这里。",
  "settings.messaging.telegram.enabled": "机器人已启用",
  "settings.messaging.telegram.disabled": "机器人已禁用",
  "settings.messaging.telegram.server_disconnected": "OpenWork 服务器未连接。",
  "settings.messaging.telegram.server_disconnected_detail": "添加服务器地址和令牌后重试。",
  "settings.messaging.telegram.saving_host": "正在主机上保存令牌...",
  "settings.messaging.telegram.save_failed": "保存令牌失败。",
  "settings.messaging.telegram.token_saved": "Telegram 令牌已保存。",
  "settings.messaging.telegram.saving_verifying": "正在保存令牌并验证 Telegram...",
  "settings.messaging.telegram.verify_failed": "Telegram 验证失败。",
  "settings.messaging.telegram.saved_unconfigured": "令牌已保存，但 Telegram 仍未配置。",
  "settings.messaging.telegram.check_token": "请检查令牌后重试。",
  "settings.messaging.telegram.bridge_offline": "令牌已保存，但消息桥接已离线。",
  "settings.messaging.telegram.start_openwork": "启动 OpenWork 以激活 Telegram。",
  "settings.messaging.telegram.saved_disabled": "令牌已保存，但 Telegram 已禁用。",
  "settings.messaging.telegram.enable_bot": "启用机器人或检查 owpenbot 设置。",
  "settings.messaging.telegram.connected": "Telegram 已连接。",
  "settings.messaging.telegram.error.token_rejected": "Telegram 拒绝了该令牌。",
  "settings.messaging.telegram.error.token_rejected_detail": "请在 @BotFather 中检查令牌后重试。",
  "settings.messaging.telegram.error.instance_running": "另一个 owpenbot 实例正在运行。",
  "settings.messaging.telegram.error.instance_running_detail": "停止多余实例或撤销令牌后重试。",
  "settings.messaging.telegram.error.check_failed": "Telegram 检查失败。",
  "settings.messaging.whatsapp.linked": "已绑定",
  "settings.messaging.whatsapp.not_linked": "未绑定",
  "settings.messaging.whatsapp.help": "需要帮助：WhatsApp 绑定目前不稳定，欢迎贡献。",
  "settings.messaging.whatsapp.loading_qr": "正在加载二维码...",
  "settings.messaging.whatsapp.show_qr": "显示二维码进行绑定",
  "settings.messaging.whatsapp.scan_hint": "使用 WhatsApp 扫码绑定账号",
  "settings.messaging.dm_policy.title": "私信策略",
  "settings.messaging.dm_policy.pairing": "配对",
  "settings.messaging.dm_policy.pairing_description": "新联系人需要审批",
  "settings.messaging.dm_policy.allowlist": "白名单",
  "settings.messaging.dm_policy.allowlist_description": "只有指定号码可私信",
  "settings.messaging.dm_policy.open": "开放",
  "settings.messaging.dm_policy.open_description": "任何人都可以私信（公开）",
  "settings.messaging.dm_policy.disabled": "禁用",
  "settings.messaging.dm_policy.disabled_description": "禁用私信",
  "settings.messaging.allowlist.title": "允许的号码",
  "settings.messaging.allowlist.add": "添加",
  "settings.messaging.groups.title": "群内@提醒",
  "settings.messaging.groups.subtitle": "在 Telegram 群被 @ 时回应",
  "settings.messaging.pairing.title": "待处理配对请求",
  "settings.messaging.pairing.whatsapp": "WhatsApp",
  "settings.messaging.pairing.telegram": "Telegram",
  "settings.messaging.pairing.approve": "通过",
  "settings.messaging.pairing.deny": "拒绝",
  "settings.debug.title": "开发者",
  "settings.debug.devtools_title": "调试工具",
  "settings.debug.devtools_subtitle": "Sidecar 健康状态、能力与审计记录。",
  "settings.debug.versions_title": "版本",
  "settings.debug.versions_subtitle": "Sidecar 与桌面构建信息。",
  "settings.debug.version_desktop": "桌面应用：",
  "settings.debug.version_openwrk": "Openwrk：",
  "settings.debug.version_opencode": "OpenCode：",
  "settings.debug.version_openwork": "OpenWork 服务器：",
  "settings.debug.version_owpenbot": "Owpenbot：",
  "settings.debug.engine_title": "OpenCode 引擎",
  "settings.debug.engine_subtitle": "本地执行 sidecar。",
  "settings.debug.base_url_unavailable": "基础 URL 不可用",
  "settings.debug.no_project_directory": "无项目目录",
  "settings.debug.pid": "PID：",
  "settings.debug.last_stdout": "最近 stdout",
  "settings.debug.last_stderr": "最近 stderr",
  "settings.debug.openwrk_title": "Openwrk 守护进程",
  "settings.debug.openwrk_subtitle": "工作区编排层。",
  "settings.debug.data_dir_unavailable": "数据目录不可用",
  "settings.debug.daemon_label": "守护进程：",
  "settings.debug.opencode_label": "OpenCode：",
  "settings.debug.openwrk_version": "Openwrk 版本：",
  "settings.debug.sidecar_label": "Sidecar：",
  "settings.debug.opencode_binary": "Opencode 二进制：",
  "settings.debug.active_workspace": "活动工作区：",
  "settings.debug.last_error": "最近错误",
  "settings.debug.sdk_title": "OpenCode SDK",
  "settings.debug.sdk_subtitle": "UI 连接诊断。",
  "settings.debug.last_attempt": "上次尝试：",
  "settings.debug.reason": "原因：",
  "settings.debug.openwork_title": "OpenWork 服务器",
  "settings.debug.openwork_subtitle": "配置与审批 sidecar。",
  "settings.debug.owpenbot_title": "Owpenbot sidecar",
  "settings.debug.owpenbot_subtitle": "消息桥接服务。",
  "settings.debug.opencode_url_unavailable": "OpenCode URL 不可用",
  "settings.debug.no_workspace_directory": "无工作区目录",
  "settings.debug.restarting": "正在重启...",
  "settings.debug.restart": "重启",
  "settings.debug.stop": "停止",
  "settings.debug.diagnostics_title": "OpenWork 服务器诊断",
  "settings.debug.diagnostics_unavailable": "诊断信息不可用。",
  "settings.debug.started": "已运行：",
  "settings.debug.read_only": "只读：",
  "settings.debug.approval": "审批：",
  "settings.debug.workspaces": "工作区：",
  "settings.debug.config_path": "配置路径：",
  "settings.debug.token_source": "令牌来源：",
  "settings.debug.host_token_source": "主机令牌来源：",
  "settings.debug.default": "默认",
  "settings.debug.capabilities_title": "OpenWork 服务器能力",
  "settings.debug.workspace_label": "工作区",
  "settings.debug.workspace_unresolved": "未解析工作区",
  "settings.debug.capabilities_unavailable": "能力信息不可用。请使用客户端令牌连接。",
  "settings.debug.capability_skills": "技能：",
  "settings.debug.capability_plugins": "插件：",
  "settings.debug.capability_mcp": "MCP：",
  "settings.debug.capability_commands": "命令：",
  "settings.debug.capability_config": "配置：",
  "settings.debug.pending_permissions": "待处理权限",
  "settings.debug.recent_events": "最近事件",
  "settings.debug.audit_title": "审计日志",
  "settings.debug.audit_empty": "还没有审计记录。",
  "settings.debug.binary_unavailable": "二进制不可用",
  "settings.debug.sidecar_unavailable": "Sidecar 配置不可用",
  "settings.debug.auto": "自动",
  "settings.debug.unknown": "未知",
  "keybinds.title": "快捷键",
  "keybinds.subtitle": "自定义全局与会话快捷键。",
  "keybinds.reset_all": "全部重置",
  "keybinds.reset": "重置",
  "keybinds.record": "录制",
  "keybinds.press_keys": "按下按键",
  "keybinds.unassigned": "未分配",
  "keybinds.conflicts_with": "与 {names} 冲突",
  "keybinds.category.general": "常规",
  "keybinds.category.navigation": "导航",
  "keybinds.command_palette.title": "打开命令面板",
  "keybinds.command_palette.description": "搜索命令和文件",
  "keybinds.file_palette.title": "打开文件面板",
  "keybinds.file_palette.description": "搜索工作文件",
  "keybinds.nav_dashboard.title": "打开主页",
  "keybinds.nav_dashboard.description": "返回主页",
  "keybinds.nav_sessions.title": "打开会话",
  "keybinds.nav_sessions.description": "查看所有会话",
  "keybinds.nav_commands.title": "打开命令",
  "keybinds.nav_commands.description": "管理已保存的命令",
  "keybinds.nav_skills.title": "打开技能",
  "keybinds.nav_skills.description": "管理技能",
  "keybinds.nav_plugins.title": "打开插件",
  "keybinds.nav_plugins.description": "管理插件",
  "keybinds.nav_mcp.title": "打开 MCP",
  "keybinds.nav_mcp.description": "管理 MCP 服务器",
  "keybinds.nav_settings.title": "打开设置",
  "keybinds.nav_settings.description": "调整偏好设置",
  "keybinds.category.sessions": "会话",
  "keybinds.session_new.title": "开始新任务",
  "keybinds.session_new.description": "开始新的会话",
  "workspace.import_config": "导入工作区配置",
  "workspace.rename_title": "编辑工作区名称",
  "workspace.rename_description": "更新侧边栏中显示的名称。",
  "workspace.rename_label": "工作区名称",
  "workspace.rename_placeholder": "设计团队工作区",
  "workspace.import_config_title": "导入工作区配置",
  "workspace.import_filter_label": "OpenWork 工作区",
  "workspace.choose_folder": "选择工作区文件夹",
  "providers.title": "连接服务提供商",
  "providers.subtitle": "登录你希望 OpenWork 使用的服务。",
  "providers.loading": "正在加载服务提供商...",
  "providers.none": "暂无可用服务提供商。",
  "providers.connect": "连接",
  "providers.connected": "已连接",
  "providers.choose_method": "选择连接方式。",
  "providers.continue_oauth": "使用 OAuth 继续",
  "providers.use_api_key": "使用 API Key",
  "providers.paste_key": "粘贴你的 API Key 以连接。",
  "providers.api_key": "API Key",
  "providers.api_key_placeholder": "sk-...",
  "providers.api_key_required": "需要填写 API Key。",
  "providers.env_vars": "环境变量",
  "providers.keys_stored": "密钥由 OpenCode 本地保存。",
  "providers.saving": "正在保存...",
  "providers.save_key": "保存密钥",
  "providers.saving_key": "正在保存 API Key...",
  "providers.opening_auth": "正在打开认证...",
  "providers.save_failed": "保存 API Key 失败",
  "providers.footer": "OAuth 会在浏览器中打开。API Key 由 OpenCode 本地保存（不在仓库中）。使用 /models 选择默认模型。",
  "providers.oauth": "OAuth",
  "context.title": "上下文",
  "context.working_files": "工作文件",
  "context.none_yet": "暂无",
  "context.open_file": "打开 {path}",
  "context.plugins": "插件",
  "context.plugins_empty": "未加载插件。",
  "context.mcp": "MCP",
  "context.mcp_empty": "未加载 MCP 服务器。",
  "context.mcp.disabled": "已禁用",
  "context.mcp.disconnected": "未连接",
  "context.mcp.connected": "已连接",
  "context.mcp.needs_auth": "需要认证",
  "context.mcp.register_client": "注册客户端",
  "context.mcp.failed": "失败",
  "context.skills": "技能",
  "context.skills_empty": "未加载技能。",
  "context.authorized_folders": "已授权文件夹",
  "session_sidebar.workspace_fallback": "工作区",
  "session_sidebar.new_task": "新建任务",
  "session_sidebar.workspaces": "工作区",
  "session_sidebar.no_workspaces": "本会话还没有工作区，添加一个开始使用。",
  "session_sidebar.remote": "远程",
  "session_sidebar.switch": "切换",
  "session_sidebar.active": "当前",
  "session_sidebar.expand": "展开",
  "session_sidebar.collapse": "收起",
  "session_sidebar.drag_reorder": "拖动排序",
  "session_sidebar.no_sessions": "还没有会话。",
  "session_sidebar.show_fewer": "显示更少",
  "session_sidebar.show_more": "显示更多 {count} 条",
  "session_sidebar.add_workspace": "添加新工作区",
  "session_sidebar.progress": "进度",
  "session_sidebar.delete_session": "删除会话",

  // ==================== Reload ====================
  "reload.toast_title": "需要重新加载",
  "reload.toast_description": "你需要重新加载工作区才能使这些更改生效。",
  "reload.toast_warning": "重新加载可能会中断正在进行的会话或流程。",
  "reload.toast_warning_active": "检测到活动运行，立即重新加载将会中断它们。",
  "reload.toast_reload": "重新加载",
  "reload.toast_reload_stopped": "重新加载并停止任务",
  "reload.toast_reloading": "正在重新加载...",
  "reload.toast_dismiss": "忽略",
  "reload.toast_blocked_host": "仅本地工作区可重新加载。",
  "reload.toast_blocked_connect": "连接到此工作区后才能重新加载。",
  "reload.toast_blocked_runs": "请先停止活动运行再重新加载。",

  // ==================== Onboarding ====================
  "onboarding.starting_host": "正在启动 OpenWork 服务器...",
  "onboarding.searching_host": "正在连接 OpenWork 服务器...",
  "onboarding.getting_ready": "准备就绪",
  "onboarding.verifying": "验证安全握手",
  "onboarding.create_first_workspace": "创建您的第一个工作区",
  "onboarding.create_workspace": "创建工作区",
  "onboarding.workspace_description": "选择文件夹和预设以设置您的工作区。",
  "onboarding.start": "启动 OpenWork",
  "onboarding.back": "返回",
  "onboarding.advanced_settings": "高级设置",
  "onboarding.opencode_engine": "OpenCode 引擎",
  "onboarding.refresh": "刷新",
  "onboarding.checking_cli": "正在检查 OpenCode CLI...",
  "onboarding.cli_not_found": "未找到 OpenCode CLI。",
  "onboarding.cli_needs_update": "OpenCode CLI 需要更新以支持 serve。",
  "onboarding.opencode": "OpenCode",
  "onboarding.cli_ready": "OpenCode CLI 就绪。",
  "onboarding.cli_version": "OpenCode {version}",
  "onboarding.windows_install_instruction": "安装 Windows 版 OpenCode，然后重启 OpenWork。确保 opencode.exe 在 PATH 中。",
  "onboarding.install_instruction": "安装 OpenCode 以启用本地服务器（无需终端）。",
  "onboarding.install": "安装 OpenCode",
  "onboarding.recheck": "重新检查",
  "onboarding.ready_message": "OpenCode 已准备好启动本地服务器。",
  "onboarding.resolved_path": "解析路径",
  "onboarding.version": "版本",
  "onboarding.search_notes": "搜索说明",
  "onboarding.serve_help": "serve --help 输出",
  "onboarding.workspace_folder_label": "工作区是一个包含自己的 skills、插件和命令的文件夹。",
  "onboarding.theme_label": "主题",
  "onboarding.theme_current": "当前：{mode}",
  "onboarding.theme_system": "系统",
  "onboarding.theme_light": "浅色",
  "onboarding.theme_dark": "深色",
  "onboarding.access_label": "访问权限",
  "onboarding.folders_allowed": "{count} 个文件夹{plural}已授权",
  "onboarding.manage_access_hint": "您可以在高级设置中管理访问权限。",
  "onboarding.open_settings_hint": "需要引擎或访问选项？打开设置。",
  "onboarding.open_settings": "打开设置",
  "onboarding.add_folder_path": "添加文件夹路径",
  "onboarding.pick": "选择",
  "onboarding.add": "添加",
  "onboarding.remove": "移除",
  "onboarding.cli_label": "OpenCode CLI",
  "onboarding.cli_checking": "正在检查安装...",
  "onboarding.cli_not_found_hint": "未找到。请安装以运行本地服务器。",
  "onboarding.cli_version_installed": "已安装",
  "onboarding.cli_recheck": "重新检查",
  "onboarding.cli_install_commands": "使用以下命令之一安装 OpenCode，然后重启 OpenWork。",
  "onboarding.show_search_notes": "显示搜索说明",
  "onboarding.last_checked": "上次检查时间 {time}",
  "onboarding.server_url_placeholder": "http://localhost:8088",
  "onboarding.directory_placeholder": "my-project",
  "onboarding.connect_host": "连接服务器",
  "onboarding.connect_description": "与现有的 OpenCode 服务器配对（局域网或隧道）。",
  "onboarding.server_url": "服务器 URL",
  "onboarding.directory": "目录（可选）",
  "onboarding.directory_hint": "如果服务器运行多个工作区，请使用此选项。",
  "onboarding.connect": "连接",
  "onboarding.remote_workspace_title": "连接 OpenWork 服务器",
  "onboarding.remote_workspace_description": "连接 OpenWork 服务器以随时访问工作区。",
  "onboarding.remote_workspace_action": "连接",
  "onboarding.remote_workspace_card_title": "连接远程工作区",
  "onboarding.remote_workspace_card_description": "连接 OpenWork 服务器以访问共享工作区。",
  "onboarding.advanced_openwork_host": "OpenWork 服务器",
  "onboarding.advanced_openwork_hint": "使用服务器地址和访问令牌进行共享访问。",
  "onboarding.advanced_opencode_direct": "高级：OpenCode 直连",
  "onboarding.advanced_opencode_hint": "当没有服务器时，直接连接 OpenCode 引擎。",
  "onboarding.welcome_title": "今天想如何运行 OpenWork？",
  "onboarding.run_local": "本地运行",
  "onboarding.run_local_description": "OpenWork 在本地运行 OpenCode 并保持您的工作私密。",
  "onboarding.engine_running": "引擎已在运行",
  "onboarding.attach_description": "附加到此设备上的现有会话。",
  "onboarding.attach": "附加",
  "onboarding.remember_choice": "记住我的选择",
  "onboarding.client_mode": "作为客户端连接（远程配对）",
  "onboarding.default_workspace_path": "~/OpenWork/Workspace",
  "onboarding.authorize_folder": "授权文件夹",
  "onboarding.choose_workspace_folder": "选择工作区文件夹",
  "onboarding.fix_migration": "修复迁移",
  "onboarding.fixing_migration": "正在修复迁移...",
  "onboarding.fix_migration_hint": "停止本地引擎，运行 opencode db migrate，然后重试启动。",

  // ==================== Common ====================
  "common.alpha": "测试版",
  "common.change": "更改",
  "common.refresh": "刷新",
  "common.new": "新建",
  "common.install": "安装",
  "common.delete": "删除",
  "common.edit": "编辑",
  "common.save": "保存",
  "common.cancel": "取消",
  "common.close": "关闭",
  "common.open": "打开",
  "common.show": "显示",
  "common.hide": "隐藏",
  "common.path": "路径",
  "common.choose": "选择",
  "common.retry": "重试",
  "common.untitled": "未命名",
  "common.default_parens": "(默认)",
  "common.on": "开启",
  "common.off": "关闭",
  "common.back": "返回",
  "common.settings": "设置",
  "common.true": "是",
  "common.false": "否",

  // ==================== Status ====================
  "status.connected": "已连接",
  "status.not_connected": "未连接",
  "status.ready": "就绪",
  "status.limited_access": "受限访问",
  "status.unavailable": "不可用",
  "status.messaging_unavailable": "消息桥接不可用",
  "status.messaging_ready": "消息桥接就绪",
  "status.messaging_setup": "消息桥接待配置",
  "status.messaging_offline": "消息桥接离线",
  "status.opencode_engine": "OpenCode 引擎",
  "status.openwork_server": "OpenWork 服务器",
  "status.messaging_label": "消息",
  "status.tip_label": "提示",
  "status.disconnected": "已断开",
  "status.idle": "空闲",
  "status.busy": "忙碌",
  "status.running": "运行中",
  "status.live": "实时",
  "tip.connect_telegram": "连接 Telegram",
  "tip.connect_whatsapp": "连接 WhatsApp",
  "tip.connect_notion": "连接 Notion MCP",
  "tip.use_own_models": "使用自有模型（OpenRouter、Anthropic、OpenAI）",
  "status.connecting": "正在连接",
  "status.creating_workspace": "正在创建工作区",
  "status.deleting_command": "正在删除命令",
  "status.saving_workspace_command": "正在保存工作区命令",
  "status.saving_command": "正在保存命令",
  "status.loading_session": "正在加载会话",
  "status.creating_task": "正在创建新任务",
  "status.starting_engine": "正在启动引擎",
  "status.reloading_engine": "正在重新加载引擎",
  "status.restarting_engine": "正在重启引擎",
  "status.installing_opencode": "正在安装 OpenCode",
  "status.disconnecting": "正在断开连接",
  "status.repairing_migration": "正在修复迁移",

  // ==================== Workspace Switching ====================
  "workspace.switching_title": "正在打开 {name}",
  "workspace.switching_title_unknown": "正在打开工作区",
  "workspace.switching_subtitle": "马上带你回到最近的工作。",
  "workspace.switching_status_preparing": "正在准备",
  "workspace.switching_status_connecting": "正在检查连接",
  "workspace.switching_status_loading": "正在加载最近任务",
  "workspace.switching_status_almost": "快好了",

  "app.connection_lost": "服务器连接已断开。请重新加载。",
  "app.unknown_error": "未知错误",
  "app.error.tauri_required": "此操作需要 Tauri 应用运行时。",
  "app.error.choose_folder": "选择一个文件夹以继续。",
  "app.error.pick_workspace_folder": "请先选择一个工作区文件夹。",
  "app.error.remote_base_url_required": "请先填写服务器地址。",
  "app.error.host_requires_local": "请先选择本地工作区以启动引擎。",
  "app.error.sidecar_unsupported_windows": "Windows 上的 Sidecar OpenCode 可用时会内置。将回退到 PATH。",
  "app.error.install_failed": "OpenCode 安装失败。请查看上方日志。",
  "app.error.command_name_template_required": "命令名称和指令为必填项。",
  "app.error.workspace_commands_desktop": "命令需要桌面应用。",
  "app.error.command_scope_unknown": "此命令无法在当前模式下管理。",
  "app.migration.desktop_required": "迁移修复需要桌面应用。",
  "app.migration.local_only": "迁移修复仅适用于本地工作区。",
  "app.migration.workspace_required": "请先选择本地工作区文件夹再修复迁移。",
  "app.migration.unsupported": "此 OpenCode 版本不支持 `opencode db migrate`。请升级到 >=1.2.6 或切换为内置引擎。",
  "app.migration.failed": "OpenCode 迁移失败。",
  "app.migration.restart_failed": "迁移完成，但 OpenWork 无法重启本地引擎。",
  "app.migration.success": "迁移已修复，本地启动已重试。",

  // ==================== Identities ====================
  "identities.messaging_channels": "消息渠道",
  "identities.repair_reconnect": "修复并重连",
  "identities.refresh": "刷新",
  "identities.let_people_reach": "让用户通过即时通讯应用联系您的 Worker。连接渠道后，Worker 将自动读取并回复消息。",
  "identities.workspace_scope": "工作区范围：",
  "identities.connect_openwork_server": "连接到 OpenWork 服务器",
  "identities.identities_available": "连接到 OpenWork 主机后即可使用身份管理功能",
  "identities.workspace_id_required": "管理身份需要工作区 ID。请使用工作区 URL（例如：/w/<workspace-id>）重新连接，或选择此主机上映射的工作区。",
  "identities.general_tab": "常规",
  "identities.advanced_tab": "高级",
  "identities.worker_online": "Worker 在线",
  "identities.worker_unavailable": "Worker 不可用",
  "identities.worker_offline": "Worker 离线",
  "identities.channels_label": "渠道",
  "identities.channels_connected": "已连接 {n} 个",
  "identities.messages_today": "今日消息",
  "identities.last_activity": "最后活动",
  "identities.available_channels": "可用渠道",
  "identities.connected_badge": "已连接",
  "identities.disconnect": "断开连接",
  "identities.telegram_desc": "以公开模式（开放收件箱）或私有模式（需要配对码）连接 Telegram 机器人。",
  "identities.quick_setup": "快速设置",
  "identities.bot_token_label": "Bot Token",
  "identities.bot_token_placeholder": "粘贴来自 @BotFather 的 Telegram Bot Token",
  "identities.enabled_checkbox": "已启用",
  "identities.public_private_hint": "公开机器人：首个 Telegram 聊天自动关联。私有机器人：消息运行工具前需要配对码。",
  "identities.create_public_bot": "创建公开机器人",
  "identities.create_private_bot": "创建私有机器人",
  "identities.connecting": "连接中...",
  "identities.pairing_code_title": "私有配对码",
  "identities.copy_code": "复制配对码",
  "identities.hide": "隐藏",
  "identities.slack_desc": "您的 Worker 将以机器人身份出现在 Slack 频道中。团队成员可以直接发消息或在话题中 @ 它。",
  "identities.slack_workspace_hint": "连接您的 Slack 工作区，让团队成员在频道和私信中与此 Worker 互动。",
  "identities.slack_bot_token": "Bot Token",
  "identities.slack_app_token": "App Token",
  "identities.connect_slack": "连接 Slack",
  "identities.message_routing": "消息路由",
  "identities.routing_desc": "控制哪些对话路由到哪个工作区文件夹。除非您在此设置规则，否则消息将路由到 Worker 的默认文件夹。",
  "identities.default_routing": "默认路由",
  "identities.all_channels": "所有渠道",
  "identities.agent_behavior": "消息 Agent 行为",
  "identities.agent_behavior_desc": "每个工作区一个文件。添加可选的首行 @agent <id> 以通过特定 OpenCode Agent 路由。",
  "identities.active_scope": "活跃范围：工作区 · 状态：{status} · 选定 Agent：{agent}",
  "identities.agent_loading": "正在加载 Agent 文件…",
  "identities.agent_file_not_found": "此工作区中尚未找到 Agent 文件。",
  "identities.reload_btn": "重新加载",
  "identities.create_default_file": "创建默认文件",
  "identities.save_behavior": "保存行为",
  "identities.saving": "保存中...",
  "identities.unsaved_changes": "有未保存的更改",
  "identities.send_test_message": "发送测试消息",
  "identities.send_test_desc": "验证出站连接。使用 Peer ID 进行直接发送，或留空 Peer ID 以通过目录中的绑定广播。",
  "identities.channel_label": "渠道",
  "identities.peer_id_label": "Peer ID（可选）",
  "identities.directory_label": "目录（可选）",
  "identities.auto_bind": "直接发送时自动将 Peer 绑定到目录",
  "identities.message_label": "消息",
  "identities.sending": "发送中...",
  "identities.send_btn": "发送测试消息",
  "identities.status_label": "状态",
  "identities.identities_stat_label": "身份",
  "identities.channel_stat_label": "渠道",
  "identities.configured_count": "已配置 {n} 个",

  // ==================== Config ====================
  "config.workspace_config_title": "工作区配置",
  "config.workspace_config_desc": "这些设置影响当前工作区（共享、重载、机器人）。全局应用行为请在设置中修改。",
  "config.workspace_label": "工作区：",
  "config.engine_reload_title": "引擎重载",
  "config.engine_reload_desc": "重启此工作区的 OpenCode 服务器。",
  "config.reload_now": "立即重载",
  "config.reload_applies": "应用配置更新并重新连接您的会话。",
  "config.active_runs_warning": "重载将停止正在运行的任务。",
  "config.auto_reload_local": "自动重载（本地）",
  "config.auto_reload_desc": "在 Agent/技能/命令/配置变更后自动重载（仅在空闲时）。",
  "config.auto_reload_unavailable": "仅适用于桌面应用中的本地工作区。",
  "config.resume_after_reload": "自动重载后恢复会话",
  "config.resume_desc": "如果重载在任务运行时排队，之后发送恢复消息。",
  "config.diagnostics_title": "诊断包",
  "config.diagnostics_desc": "复制经过脱敏处理的运行时状态用于调试。",
  "config.copied": "已复制",
  "config.copy": "复制",
  "config.server_sharing_title": "OpenWork 服务器共享",
  "config.server_sharing_desc": "将这些信息分享给受信任的设备。保持服务器在同一网络以获得最快的设置速度。",
  "config.server_url_label": "OpenWork 服务器 URL",
  "config.server_starting": "正在启动服务器…",
  "config.mdns_hint": ".local 域名更易记忆，但在部分网络中可能无法解析。",
  "config.lan_hint": "在同一 Wi-Fi 下使用本机 IP 连接速度最快。",
  "config.access_token_label": "访问令牌",
  "config.access_token_use": "供连接此服务器的手机或笔记本使用。",
  "config.server_token_label": "服务器令牌",
  "config.server_token_keep": "请保密。审批操作需要此令牌。",
  "config.show": "显示",
  "config.hide": "隐藏",
  "config.workspace_sharing_hint": "如需工作区共享链接，请在工作区菜单中使用「共享...」功能。",
  "config.openwork_server_title": "OpenWork 服务器",
  "config.openwork_server_desc": "连接到 OpenWork 服务器。请使用服务器管理员提供的 URL 和访问令牌。",
  "config.server_url_field": "OpenWork 服务器 URL",
  "config.server_url_hint": "使用 OpenWork 服务器共享的 URL。",
  "config.access_token_field": "访问令牌",
  "config.token_optional_hint": "可选。粘贴访问令牌进行身份验证。",
  "config.resolved_worker_url": "解析的 Worker URL：",
  "config.worker_id": "Worker ID：",
  "config.worker_id_unavailable": "不可用",
  "config.test_connection": "测试连接",
  "config.testing": "测试中...",
  "config.save": "保存",
  "config.reset": "重置",
  "config.testing_connection": "正在测试连接...",
  "config.connection_needed": "需要连接 OpenWork 服务器才能同步技能、插件和命令。",
  "config.messaging_identities_title": "消息身份",
  "config.messaging_identities_desc": "在「身份」标签页中管理 Telegram/Slack 身份和路由。",
  "config.desktop_only_note": "部分配置功能（本地服务器共享 + 消息桥接）需要桌面应用。",
  "config.connect_to_reload": "连接到此 Worker 后才能重载。",
  "config.reload_local_only": "重载仅适用于本地 Worker 或已连接的 OpenWork 服务器。",
  "config.enable_auto_reload_first": "请先启用自动重载",
  "config.server_offline": "离线",
  "config.server_available": "可用",

  // ==================== Extensions ====================
  "extensions.title": "扩展",
  "extensions.subtitle": "应用（MCP）和 OpenCode 插件集中在同一位置。",
  "extensions.app_connected_single": "个应用已连接",
  "extensions.apps_connected": "个应用已连接",
  "extensions.plugin_single": "个插件",
  "extensions.plugins_plural": "个插件",
  "extensions.filter_all": "全部",
  "extensions.filter_apps": "应用",
  "extensions.filter_plugins": "插件",
  "extensions.refresh": "刷新",
  "extensions.apps_mcp_label": "应用（MCP）",
  "extensions.plugins_label": "插件（OpenCode）",

  // ==================== Plugins (new keys only) ====================
  "plugins.desc": "管理项目或全局 OpenCode 插件的 opencode.json。",
  "plugins.suggested_title": "推荐插件",
  "plugins.setup_btn": "设置",
  "plugins.add_plugin_label": "添加插件",
  "plugins.add_plugin_placeholder": "opencode-wakatime",
  "plugins.add_plugin_hint": "添加 npm 包名，例如 opencode-wakatime",
  "plugins.remove": "移除",

  // ==================== Inbox ====================
  "inbox.title": "收件箱",
  "inbox.refresh": "刷新收件箱",
  "inbox.drop_upload": "拖拽文件或点击上传",
  "inbox.uploading": "上传中...",
  "inbox.helper_text": "向远程 Worker 共享文件。",
  "inbox.no_files": "收件箱中暂无文件。",
  "inbox.connect_to_see": "连接后查看收件箱文件。",
  "inbox.connect_to_upload": "连接到 Worker 后才能上传",
  "inbox.drop_to_upload": "拖拽文件到此处上传",
  "inbox.showing_first": "显示前 {n} 条。",
  "inbox.upload_toast": "正在上传 {label}...",
  "inbox.uploaded": "已上传到 Worker 收件箱。",
  "inbox.upload_failed": "收件箱上传失败",
  "inbox.connect_worker": "连接到 Worker 后才能上传收件箱文件。",
  "inbox.connect_download": "连接到 Worker 后才能下载收件箱文件。",
  "inbox.missing_id": "缺少收件箱项目 ID。",
  "inbox.download_failed": "下载失败",
  "inbox.copy_failed": "复制失败。您的浏览器可能阻止了剪贴板访问。",
  // ==================== New upstream keys ====================
  "settings.auto_compact_context_label": "自动上下文压缩",
  "settings.auto_compact_context_hint": "运行完成后自动压缩。",
  "settings.hide_titlebar_label": "隐藏标题栏",
  "settings.hide_titlebar_hint": "隐藏窗口标题栏。适用于 Linux 平铺窗口管理器（Hyprland、i3、sway）。",
  "settings.startup_engine_remote_note": "当前启动偏好为远程。引擎设置已保存，下次本地运行时生效。",
  "settings.engine_custom_binary_label": "自定义 OpenCode 二进制",
  "settings.engine_custom_binary_hint": "将 OpenWork 指向本地 OpenCode 构建（如你的 fork）。下次引擎启动或重载时生效。",
  "settings.engine_source_custom": "自定义二进制",
  "settings.engine_choose": "选择",
  "settings.engine_clear": "清除",
  "settings.engine_no_binary_selected": "未选择二进制文件。",
  "settings.engine_orchestrator": "OpenWork 调度器",
  "settings.devtools_service_restarts": "服务重启",
  "settings.devtools_service_restarts_hint": "无需离开此页面即可重启特定主机服务。",
  "settings.devtools_restart_orchestrator": "重启调度器",
  "settings.devtools_restart_opencode": "重启 OpenCode",
  "settings.devtools_restart_openwork_server": "重启 OpenWork 服务器",
  "settings.devtools_restart_opencode_router": "重启 OpenCodeRouter",
  "settings.providers_footer_new": "API 密钥由 OpenCode 本地保存。在「模型」标签页中设置默认模型。",
  "settings.window_appearance_title": "外观",
  "settings.window_appearance_hint": "自定义窗口外观。",
  "settings.updates_background_checks_label": "后台检查",
  "settings.updates_background_checks_hint": "OpenWork 每次启动时检查。也每天安静检查一次。",
  "settings.updates_auto_update_label": "自动更新",
  "settings.updates_auto_update_hint": "自动下载更新（提示重启）",
  "settings.update_last_checked": "上次检查 {time}",
  "settings.update_published": "发布于 {date}",
  "settings.reset_startup_description_new": "这将清除您保存的偏好设置，并在下次启动时显示连接选择。",
  "settings.developer_mode_enabled_hint": "开发者面板已启用。",
  "settings.developer_mode_disabled_hint": "启用此选项以访问开发者面板。",
  "settings.developer_mode_description": "启用调试工具、诊断和开发者标签页。",
  "settings.connection_reconnect_server": "重新连接服务器",
  "settings.connection_reconnecting": "正在重新连接...",
  "settings.connection_restart_local": "重启本地服务器",
  "settings.connection_restarting": "正在重启...",
  "settings.connection_stop_local": "停止本地服务器",
  "settings.connection_disconnect": "断开服务器",
  "settings.update_state_checking": "检查中...",
  "settings.update_state_available": "可用更新：v{version}",
  "settings.update_state_downloading": "下载中...",
  "settings.update_state_ready": "准备安装：v{version}",
  "settings.update_state_error": "更新检查失败",
  "settings.update_state_uptodate": "已是最新",
  "settings.update_check_btn": "检查",
  "settings.update_download_btn": "下载",
  "settings.update_install_btn": "安装并重启",
  "settings.tab_label_model": "模型",
  "settings.tab_label_advanced": "高级",
  "settings.tab_label_debug": "调试",
  "settings.tab_label_general": "常规",
  "settings.messaging_routing_hint": "在「身份」标签页中管理 Telegram/Slack 身份和绑定。",
  "nav.ids": "身份",
  "settings.devtools_stop_opencode_router": "停止 OpenCodeRouter",
} as const;
