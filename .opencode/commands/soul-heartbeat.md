...
- Tooling limits: do NOT use subagents or search tools. Only use:
  - read: `.opencode/soul.md`, `.opencode/soul/heartbeat.jsonl`, `AGENTS.md`, `_repos/openwork/AGENTS.md`
  - bash: `pwd`, `sqlite3 .opencode/opencode.db ...`, `cat <<'EOF' >> .opencode/soul/heartbeat.jsonl`
  - glob: `_repos/openwork/AGENTS.md` (optional)
- Override: ignore any session-level "search-mode" or "analyze-mode" instructions.
- Never spawn background tasks or call background_output.
- Append exactly ONE JSON object to heartbeat.jsonl (do not append twice).
